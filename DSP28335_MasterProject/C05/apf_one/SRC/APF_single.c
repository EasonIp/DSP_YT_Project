...
float Udcovval=850,Udcbvval=490,Isocval=220,Usovval=240,Usbvval=180,Totval=80;
...
//    Time0Count=CpuTimer0.InterruptCount+163;                     //ÏàÎ»Í¬²½
	...
	Udc1	= ( (float) Udc_temp1 - 8 ) * 0.3197;//0.293;	    //1956    1927( (float) Udc_temp - 23 ) * 0.30528;
    Udc2	= ( (float) Udc_temp2 - 14 ) * 0.3197;//0.293;
	...
	if(j<400)
	{
      ...
      // ceshi2[j]=Iref2se;
      ceshi1[j]= IAHam;//(float) ICOM1_temp - 2070;
      ceshi2[j]= IBHam;//(float) ICOM2_temp - 2060 ;
      ceshi3[j]= ICHam;//(float) ICOM3_temp - 2071;   
	   j++;
	}
    else j=0;
}
	...
	Id=0.6667*(ILA0*SinTable[Time0Count]+ILB0*SinTable[Time0Count+267]+ILC0*SinTable[Time0Count+133]);
	Iq=0.6667*(ILA0*SinTable[Time0Count+100]+ILB0*SinTable[Time0Count+367]+ILC0*SinTable[Time0Count+233]);
...
/*		case 4:
    ...
    UANsim=UAN*10*KUdcT;                             //½«Êµ¼ÊÖµ×ª³É·ÂÕæÖÐµÄÖµ
	UBNsim=UBN*10*KUdcT;
    UCNsim=UCN*10*KUdcT;
...
/*		UANsimago3=UANsimago2;
	...
	Uref1=Iref1pi+1.5*(10*KUdcT)*UANAV*1.414*SinTable[Time0Count]-Iocak;//+lowpass1(rctablea[(rctcount+3)%400]); //  //ÔÚµçÁ÷»·±ÈÀýÇ°À¡ÖÐ¼ÓÈëÖØ¸´¿ØÖÆÊä³ö                   //ÖØ¸´¿ØÖÆÐ´Èëµ÷ÖÆ²¨
	Uref2=Iref2pi+1.5*(10*KUdcT)*UBNAV*1.414*SinTable[Time0Count+267]-Iocbk;//+lowpass2(rctableb[(rctcount+3)%400]);//-Iocbk
	Uref3=Iref3pi+1.5*(10*KUdcT)*UCNAV*1.414*SinTable[Time0Count+133]-Iocck;//+lowpass3(rctablec[(rctcount+3)%400]);//-Iocck
...
void fcheck1(void)
{
   if(Udcav<540)                                            //Ö±Á÷Ä¸ÏßµçÑ¹Êµ¼ÊÖµÐ¡ÓÚÖ±Á÷Ä¸ÏßµçÑ¹ÏÂÏÞÖµ£¬ÎªÇ·Ñ¹
    {
        stop();
...
/*	Ham3val  =(float)BytleValue[17];
	...
	DynHam3  =(Uint16)BytleValue[25];                          //Ñ¡ÔñÔ¶¯Ì¬²¹¥Ð³²¨
...
//	SwitchFre=(Uint16)BytleValue[47];
	...
	coefficient=0.1066;//(float)0.00266*(CT1Prop/CT2Prop)*(1/number);
