...
//    Time0Count=CpuTimer0.InterruptCount+163;                     //ÏàÎ»Í¬²½
	...
	UAB     = ((float)UAB_temp - 2070)*0.53267045;      //  ((float)UAB_temp - 2051)*0.53267045;       //±ÈÀýÏµÊýÐè¸ù¾Ý´«¸ÐÆ÷ºÍ²ÉÑùµ÷ÀíµçÂ·À´¶¨
...
//  UCN		= ( (float) UCN_temp-2057 )*0.53267045;  
    ...
    ICOM1	= ( (float) ICOM1_temp - 2074 )*0.36621;  //( (float) ICOM1_temp - 2070 ) * 0.2441406; 
	ICOM2	= ( (float) ICOM2_temp - 2064 )*0.36621;  // ( (float) ICOM2_temp - 2064 ) * 0.2441406;
	ICOM3	= ( (float) ICOM3_temp - 2074 )*0.36621;  //( (float) ICOM3_temp - 2069 ) * 0.2441406; 
   ...
   Operamode=1;
	switch(Operamode)                                  //¸ù¾Ý´¥ÃþÆÁµÄÖ¸ÁîÑ¡ÔñÏàÓ¦µÄÖ¸ÁîµçÁ÷
	{
		case 1:
				   ...
				   break;
		case 2:
            	   Iref1se=20*SinTable[Time0Count+100];                          
		           Iref2se=20*SinTable[Time0Count+367];
		           Iref3se=(-20*SinTable[Time0Count+100]-20*SinTable[Time0Count+367]);
				   break;
		case 3:
