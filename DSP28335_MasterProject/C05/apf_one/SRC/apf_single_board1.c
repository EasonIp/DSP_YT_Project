...
float  Isysa,Isysb,Isysc,Isysasum=0,Isysbsum=0,Isyscsum=0,Isysaav=0,Isysbav=0,Isyscav=0;
...
float  UAN,UBN,UCN,ILA,ILB,ILC,Udc,Udc1,Udc2,Tem1,Tem2,Temax,ICOM1,ICOM2,ICOM3;      //µçÍøµçÑ¹¡¢Ö±Á÷²àµçÑ¹ºÍÎÂ¶ÈµÄÊµ¼ÊÖµºÍ¸ºÔØµçÁ÷ºÍ²¹³¥µçÁ÷µÄÊµ¼ÊÖµ                                  //
...
//    Time0Count=CpuTimer0.InterruptCount+163;                     //ÏàÎ»Í¬²½
	...
	Udc1	= ( (float) Udc_temp1 - 31 ) * 0.293;	    //1956    1927( (float) Udc_temp - 23 ) * 0.30528;
	...
	if(j<400)
	{
      ...
      // ceshi2[j]=Iref2se;
      ceshi1[j]= ILA;//(float) ICOM1_temp - 2070;
      ceshi2[j]= IAHam;//(float) ICOM2_temp - 2060 ;
      ceshi3[j]= -ICOM1+ILA;//(float) ICOM3_temp - 2071;   
	   j++;
	}
    else j=0;
}
...
//	SwitchFre=(Uint16)BytleValue[47];
	...
	coefficient=(float)0.00266*(CT1Prop/CT2Prop)*(1/number);
...
void contactordeal(void){
	...
	if(chargflag){
	if((chargcount<20000)&&(!(faultstasec.all&0x003f))){
		contactorK1=0;
		contactorK2=1;
		contactor1=0;
		contactor2=1;
