...
float  Isysa,Isysb,Isysc,Isysasum,Isysbsum,Isyscsum,Isysaav,Isysbav,Isyscav;
float  Udc1av=0,Udc2av=0;
float  THDIL,THDIsys;
Uint16 UAN_temp,UBN_temp,UCN_temp,ILA_temp,ILB_temp,ILC_temp,Tem1_temp,Tem2_temp;   //电网电压、直流侧电压和温度的采样值
Uint16 Udc_temp1,Udc_temp2,ICOM1_temp,ICOM2_temp,ICOM3_temp;                 //负载电流和补偿电流的采样值
//Uint16 IAref_temp,IBref_temp,ICref_temp; 

float  UAN,UBN,UCN,ILA,ILB,ILC,Udc,Udc1,Udc2,Tem1,Tem2,Temax,ICOM1,ICOM2,ICOM3;      //电网电压、直流侧电压和温度的实际值和负载电流和补偿电流的实际值                                  //
...
Uint16 Operamode=8;//1表示稳压 ，2定量感性，3定量容性，4定量谐波，5动态补谐波
...
float Udcovval=850,Udcbvval=490,Isocval=150,Usovval=240,Usbvval=180,Totval=80;
...
/*	for(i=0;i<700;i++)
    ...
    for(;;)
	{
		...
		if(rsorder==1)                                                //逆变启动指令
		{
			if(state==0&&!runorcount&&contactor2) start();
...
//    Time0Count=CpuTimer0.InterruptCount+163;                     //相位同步
	...
	UAN     = ((float)UAN_temp - 2060)*0.53267045;      //  ((float)UAB_temp - 2051)*0.53267045;       //比例系数需根据传感器和采样调理电路来定
	UBN		= ((float) UBN_temp-2070)*0.53267045;//( 2072 - (float) UCB_temp )*0.53267045;
    UCN		= ( (float) UCN_temp-2070 )*0.53267045;  
	Udc1	= ( (float) Udc_temp1 - 15 ) * 0.293;	    //1956    1927( (float) Udc_temp - 23 ) * 0.30528;
    Udc2	= ( (float) Udc_temp2 - 31 ) * 0.293;
	...
	ILA	    =  ( (float) ILA_temp - 2077 ) * 0.1057;   //*ct/10
	ILB  	= ( (float) ILB_temp - 2058 ) * 0.1057; 
	ILC	    = ( (float) ILC_temp - 2075 ) * 0.1057;
	Isysa   =  -ICOM1+ILA;
	Isysb   =  -ICOM2+ILB;
	Isysc   =  -ICOM3+ILC;
//     if(state){
 ...
 //      ceshi2[j]= UBN ;//(float) ICOM2_temp - 2060 ;
       ceshi3[j]=Iref3se;//(float) ICOM3_temp - 2071;   
	   j++;
	}
    else j=0;
//}
...
/*		UANsimago3=UANsimago2;
	...
	if(Udcavsim>UdcTarget*0.0027)                                 //直流电压大于690V，在指令中加入谐波指令
	{
	...
	else
	{
		...
		Iocak=0.5*(10*KUdcT)*310*SinTable[Time0Count];
		Iocbk=0.5*(10*KUdcT)*310*SinTable[Time0Count+267];
 		Iocck=0.5*(10*KUdcT)*310*SinTable[Time0Count+133];
	...
	Uref1=Iref1pi+1.5*(10*KUdcT)*310*SinTable[Time0Count]-Iocak;//+lowpass1(rctablea[(rctcount+3)%400]); //  //在电流环比例前馈中加入重复控制输出                   //重复控制写入调制波
	Uref2=Iref2pi+1.5*(10*KUdcT)*310*SinTable[Time0Count+267]-Iocbk;//+lowpass2(rctableb[(rctcount+3)%400]);//-Iocbk
	Uref3=Iref3pi+1.5*(10*KUdcT)*310*SinTable[Time0Count+133]-Iocck;//+lowpass3(rctablec[(rctcount+3)%400]);//-Iocck
...
//   *(Cmp1Write)=(Uint16)(SinTable[Time0Count] * 800+1000);
 //  *(Cmp2Write)=(Uint16)(SinTable[Time0Count+267] * 800+1000);
  // *(Cmp3Write)=(Uint16)(SinTable[Time0Count+133] * 800+1000);
	...
	Idsysav=Idsysav*0.99+Idsys*0.01;
	Iqsysav=Iqsysav*0.99+Iqsys*0.01;
    dataflag=1;
    AdcRegs.ADCTRL2.bit.RST_SEQ1=1;
	PieCtrlRegs.PIEACK.bit.ACK1=1;
...
void fcheck1(void)
{
   if(Udcav<Udcbvval)                                            //直流母线电压实际值小于直流母线电压下限值，为欠压
    {
        stop();
...
//	Hall2Prop=(float)BytleValue[43];
	if(UdcTarget!=BytleValue[42]) {                    
	UdcTarget=(float)BytleValue[42];
	KUdcT=1/UdcTarget;
	}
...
void datadeal(void)
{
	...
	THDGrid=sqrt(1-(Idsysav*Idsysav+Iqsysav*Iqsysav)/(0.666667*(Isysaav*Isysaav+Isysbav*Isysbav+Isyscav*Isyscav)));
	pllflag=0;
	}
	Gridpowfac=sqrt(Iqsysav*Iqsysav/(Idsysav*Idsysav+Iqsysav*Iqsysav));
	Loadpowfac=sqrt(Iqav*Iqav/(Idsysav*Idsysav+Iqsysav*Iqsysav));
