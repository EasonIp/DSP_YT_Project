...
float Iref1se;                                //Í¨¹ý¹¦ÄÜÑ¡ÔñµÃµ½µÄÖ¸ÁîµçÁ÷
...
float Udct=0,Udci=0,Udcpi=0; 
float Udccat=0,Udccai=0,Udccapi=0;                                    //Ö±Á÷µçÑ¹Îó²î
float Iref1dc,Iref2dc;  											  //ÎÈÑ¹µçÁ÷Ö¸Áî
float Iref1iq,Iref2iq;                                   
float Iref1se=0,Iref2se=0;
float Iref1,Iref1k=0,Iref1pq=0;   
float Iref1pi,Iref1i=0,Iref1p=0;                                              //µçÁ÷»·±ÈÀý»ý·ÖÊä³ö
float Uref1,Uref2;                                                  //µ÷ÖÆ²¨µçÑ¹Ö¸Áî
float T1=0,T2=0;
...
void main(){
    ...
    for(i=0;i<400;i++)
	{
	    SinPower[i]=1/(SinTable[i]*SinTable[i]+SinTable[i+300]*SinTable[i+300]);
	}
	for(i=0;i<400;i++) Iddata[i]=0;
	for(i=0;i<400;i++) Iqdata[i]=0;
	for(i=0;i<400;i++) UANCON[i]=0;
	for(i=0;i<400;i++) IDCON[i] =0;
	for(i=0;i<400;i++) IQCON[i] =0;
    ...
    for(;;)
	{
		...
		//Iref1k=2/(UANsim*UANsim+UBNsim*UBNsim);
		Iref1k=1000000/(UANAV*UANAV);
...
interrupt void adcisr(void)
{
	...
	UAN     = ((float)UAN_temp-2067)*0.53267045;      
	...
	switch(Operamode)                                  
	{
		case 1:
				   ...
				   break;
		case 2:
            	   Iref1se=0.001*Indval;//*SinTable[Time0Count+100];                          
				   break;
		case 3:
            	   Iref1se=0.001*Capval;//*SinTable[Time0Count+300];                          
 				   break;
		case 6:
            	   Iref1se=0.001*IAHam;                          
				   break;
	...
	UANsim=UAN*0.001;
	UANsim=1.4142*UANAV*SinTable[Time0Count]*0.001;
	Udcavsim=Udcav*0.001;
	ICOM1sim=ICOM1*0.001;                       

	fcheck2();                                          

    Udct=UdcTarget-Udcavsim;
	...
	Iref1dc=-0.01*Udcpi;//-0.311*Udcpi*SinTable[Time0Count];                
    
    if(Udcavsim>UdcTarget*0.95)                                 
	{
		Iref1iq=Iref1se;                               
	}
	else
	...
	//EP=0.5*UANsim*Iref1dc+UBNsim*Iref2dc;
	EP=Iref1dc;
	//EQ=0.5*UBNsim*Iref1iq-UANsim*Iref2iq;
	EQ=0.001*UANAV*Iref1se;

	Iref1pq=UANsim*EP+UBNsim*EQ;
    Iref1=Iref1pq*Iref1k;
	//Iref1=Iref1+Iref1se;

	Iref1p=Iref1-ICOM1sim;                                  
	Iref1i=Iref1i+0.02*Iref1p;
	Iref1pi=0.05*Iref1i+16*Iref1p; 

	if(Iref1pi>=0.35) 
	Iref1pi=0.35;
	else if(Iref1pi<=-0.35) 
	Iref1pi=-0.35;                                   

	Iref1pi=0.26*SinTable[j];
//	UANsim=Iref1pi;
...
//	if(T1>0.00005) T1=(T1-0.00005)*2;
//	else T1=0;
//	if(T2>0.00005) T2=(T2-0.00005)*2;
//	else T2=0;
	 
	*(Cmp1Write) = (Uint16)(Uref1*2000);                                
	*(Cmp2Write) = (Uint16)(Uref2*2000);

//   *(Cmp1Write)=(Uint16)( SinTable[j]*890+1000);
//   *(Cmp2Write)=(Uint16)(-SinTable[j]*890+1000);
...
void fcheck1(void)
{
/*    if(Udcav<300)                                            //Ö±Á÷Ä¸ÏßµçÑ¹Êµ¼ÊÖµÐ¡ÓÚÖ±Á÷Ä¸ÏßµçÑ¹ÏÂÏÞÖµ£¬ÎªÇ·Ñ¹
    {
        stop();
		ubvf=1;
	}
