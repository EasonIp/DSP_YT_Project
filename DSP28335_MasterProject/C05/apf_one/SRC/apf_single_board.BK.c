...
//    Time0Count=CpuTimer0.InterruptCount+163;                     //相位同步
	...
	if(j<400)
	{
       ...
       ceshi1[j]=ICOM1;//CpuTimer0.InterruptCount;//Udcavsim>UdcTarget*0.0027;
       ceshi2[j]=UAN;
 //      ceshi1[j]= UAN ;//(float) ICOM1_temp - 2070;
 //      ceshi2[j]= UBN ;//(float) ICOM2_temp - 2060 ;
       ceshi3[j]=Iref1;//(float) ICOM3_temp - 2071;   
	   j++;
	}
    else j=0;
}
...
/*		case 4:
    ...
    UANsim=UAN*10/UdcTarget;                             //将实际值转成仿真中的值
	UBNsim=UBN*10/UdcTarget;
    UCNsim=UCN*10/UdcTarget;
...
/*		UANsimago3=UANsimago2;
	...
	else
	{
		...
		Iocak=0.5*(10/UdcTarget)*310*SinTable[Time0Count];
		Iocbk=0.5*(10/UdcTarget)*310*SinTable[Time0Count+267];
 		Iocck=0.5*(10/UdcTarget)*310*SinTable[Time0Count+133];
	...
	Uref1=Iref1pi+1.5*(10/UdcTarget)*310*SinTable[Time0Count]-Iocak;//+lowpass1(rctablea[(rctcount+3)%400]); //  //在电流环比例前馈中加入重复控制输出                   //重复控制写入调制波
	Uref2=Iref2pi+1.5*(10/UdcTarget)*310*SinTable[Time0Count+267]-Iocbk;//+lowpass2(rctableb[(rctcount+3)%400]);//-Iocbk
	Uref3=Iref3pi+1.5*(10/UdcTarget)*310*SinTable[Time0Count+133]-Iocck;//+lowpass3(rctablec[(rctcount+3)%400]);//-Iocck
...
//	UBCPLL = UBC;                                               //判断相序
	CpuTimer0.InterruptCount = 0;
		...
		UBNAV=sqrt(UBNSUM);
		UCNAV=sqrt(UCNSUM);
		ILAAV=sqrt(ILASUM*0.0025);
		ILBAV=sqrt(ILBSUM*0.0025);
		ILCAV=sqrt(ILCSUM*0.0025);
	    ICOM1AV=sqrt(ICOM1SUM*0.0025);
	    ICOM2AV=sqrt(ICOM2SUM*0.0025);
	    ICOM3AV=sqrt(ICOM3SUM*0.0025);
