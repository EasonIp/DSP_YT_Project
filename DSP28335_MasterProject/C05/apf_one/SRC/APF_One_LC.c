...
interrupt void adcisr(void)
{
    ...
    if(state==0)
    {
	  ...
	  {	   
        ceshi1[j]= UAN;
        ceshi2[j]= (float)(ICOM1_temp)-2050;
        ceshi3[j]= UANAV*1.414*SinTable[Time0Count];
	    j++;
...
*/
	Iref1=Iref1dc-Capval*SinTable[Time0Count+300];
...
//    Uref1=260*SinTable[j];
	if(Uref1>350) Uref1=350;
	else if(Uref1<-350) Uref1=-350;
    Uref2=-Uref1;
	 
	*(Cmp1Write) = (Uint16)(Uref1*2.85+1000);                                
	*(Cmp2Write) = (Uint16)(Uref2*2.85+1000);
...
void fcheck1(void)
{
    if(Udcav<200)                                            //Ö±Á÷Ä¸ÏßµçÑ¹Êµ¼ÊÖµÐ¡ÓÚÖ±Á÷Ä¸ÏßµçÑ¹ÏÂÏÞÖµ£¬ÎªÇ·Ñ¹
    {
        stop();
		ubvf=1;
	}
